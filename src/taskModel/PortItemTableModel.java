...
public class PortItemTableModel extends AbstractTableModel{
	...
	protected String headers[]={"源IP","目的IP","源端口","目的端口"};
	...
	public void addNewPortMap(PortMapItem portMapItem){
		...
		mapLookUp.put(portMapItem.getSrcIP()
	...
	public Object getValueAt(int rowIndex, int coloumnIndex){
		...
		switch(coloumnIndex){
		case 0:
			value=this.portMapItemList.get(rowIndex).getSrcIP();break;
		case 1:
			value=this.portMapItemList.get(rowIndex).getSrcPort();break;
		case 2:
			value=this.portMapItemList.get(rowIndex).getDesIP();break;
		case 3:
			value=this.portMapItemList.get(rowIndex).getDesPort();break;
		}
